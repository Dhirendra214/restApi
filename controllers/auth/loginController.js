...
const loginController = {
    async login(req, res, next) {
        ...
        try {
            ...
            // Toekn
            console.log(' creating jwt ', user.email)
            const access_token = JwtService.sign({ email : user.email});
            const refresh_token = JwtService.sign({ email : user.email }, '1y', REFRESH_SECRET);
            // database whitelist
    ...
    async createTask (req , res, next) {
        ...
        try {
           ...
           User.findOneAndUpdate(
                ...
                { new: true, useFindAndModify: false }, (error, success) => {
                    if (error) {
